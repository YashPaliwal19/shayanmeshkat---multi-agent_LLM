...
def show_hint(hint, show_plots=True):
    ...
    print('output automata', output_visualization_filename)
    # exit()
...
def gen_hints(identifier, show_plots=False):
    print('identifier in gen_hints:', identifier)
    # exit()
    if False:
        pass
    # elif identifier=="?":
...
def gen_sup_hint_dfa(symbols, show_plots=False):
    ...
    print('alph in gen_sup_hint:', Traces.numbers)
    print('states in gen_sup_hint:', list(range(len(symbols)+1)))
    print('accepting in gen_sup_hint:', [len(symbols)])
    ...
    print('hint before hint.transition:', hint.accepting_states)
    hint.transitions = {
    ...
    print('hint in gen_sup_hint_dfa:', hint.init_states)
    # exit()
    return hint
...
def learn_automaton(traces_filename,
    ...
    dirname = os.path.abspath(os.path.dirname(__file__))
    print('dirname in al_utils:', dirname)
    # exit()
    show_hints = 1
    ...
    if automaton_learning_algorithm is not None:
        ...
        if automaton_learning_algorithm == "SAT":
            ...
            if sup_hint_dfas is None:
                ...
                print('in sup hint dfas is None')
                print('pyast file:', pysat_data_file)
            else:
                print('in else')
                print('sup hint dfas:', sup_hint_dfas) #=list(DFA.from_rpni(filename))
    ...
    if show_plots:
        subprocess.run(["xdot", output_visualization_filename])
    print('dfa output filename is', output_filename)
